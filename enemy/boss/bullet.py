...
##        self.fPoint_DR = [self.rect[0]+self.rect[2], self.rect[1]+self.rect[3]]
    ...
    def vLoadImage(self):
        self.imageList = []
        for i in range(1):
            imgname = os.path.abspath("data/image/boss/bullet_00%d.png"%i)
            tmpimage = pygame.image.load(imgname).convert_alpha()
            self.imageList.append(tmpimage)
        self.disableimage = pygame.image.load(os.path.abspath("data/image/boss/bullet_disable.png")).convert_alpha()
